...
Путь к json-файлу (относительный или абсолютный) можно задать в специальном файле ".env".  
Для этого в корневой папке сайта создайте файл ".env" и в нём укажите путь к json-файлу в переменной `JSON_PATH`,
Пример:
```
JSON_PATH=meta_data.json
```
Без заданного пути к файлу программа будет искать его в корневой папке под названием "meta_data.json".
